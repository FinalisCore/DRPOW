...
pkill -f "rpov2_node /tmp/rpov2_testnet_delayed/node" >/dev/null 2>&1 || true
...
mkdir -p "$ROOT/derive"
V1=$(derive_signer_id "$K1" "$ROOT/derive1.conf" "$ROOT/derive1.log" "$(pick_port 30201)")
V2=$(derive_signer_id "$K2" "$ROOT/derive2.conf" "$ROOT/derive2.log" "$(pick_port 30202)")
V3=$(derive_signer_id "$K3" "$ROOT/derive3.conf" "$ROOT/derive3.log" "$(pick_port 30203)")
...
(
  ...
  if kill -0 "$PID1" >/dev/null 2>&1 || kill -0 "$PID2" >/dev/null 2>&1 || kill -0 "$PID3" >/dev/null 2>&1; then
    echo "delayed testnet timeout: killing nodes after ${RUN_TIMEOUT_SEC}s" >&2
...
if [[ $status -ne 0 ]]; then
  echo "delayed testnet failed status=$status"
  exit "$status"
fi
...
if [[ -z "${R1:-}" || -z "${R2:-}" || -z "${R3:-}" || -z "${H1:-}" || -z "${H2:-}" || -z "${H3:-}" ]]; then
  echo "delayed_join_assert_failed: missing final_status in logs" >&2
  for f in "$ROOT"/node*.log; do
    echo "== debug $f =="
    rg "node_started|validator_set|autopropose|commit ok|catchup|drop|sync_" "$f" || true
  done
  exit 30
fi
if [[ "$R1" -le 0 || "$R2" -le 0 || "$R3" -le 0 ]]; then
  echo "delayed_join_assert_failed: no round progress r1=$R1 r2=$R2 r3=$R3" >&2
  for f in "$ROOT"/node*.log; do
    echo "== debug $f =="
    rg "node_started|validator_set|autopropose|commit ok|catchup|drop|sync_" "$f" || true
  done
...
if [[ "$H1" != "$H2" || "$H1" != "$H3" ]]; then
  echo "delayed_join_assert_failed: state_root mismatch h1=$H1 h2=$H2 h3=$H3" >&2
  for f in "$ROOT"/node*.log; do
    echo "== debug $f =="
    rg "node_started|validator_set|autopropose|commit ok|catchup|drop|sync_" "$f" || true
  done
...
if ! rg -q "catchup commit ok|commit ok round=" "$ROOT/node1.log"; then
  echo "delayed_join_assert_failed: node1 did not apply canonical commits after join" >&2
  exit 33
fi

echo "delayed_join_ok rounds=$R1,$R2,$R3 root=$H1 join_delay_sec=$JOIN_DELAY_SEC"
echo "delayed testnet finished; logs in $ROOT"
