...
DEFAULT_SEED_IP="${DEFAULT_SEED_IP:-85.217.171.168}"
DEFAULT_SEED_PORT="${DEFAULT_SEED_PORT:-19440}"
DEFAULT_SEED_PEER="${DEFAULT_SEED_IP}:${DEFAULT_SEED_PORT}"
BIND_PORT="${BIND_PORT:-19440}"
SEED_PEER="${SEED_PEER:-}"
PUBLIC_ENDPOINT="${PUBLIC_ENDPOINT:-}"
AUTOPROPOSE="${AUTOPROPOSE:-}"
AUTOPROPOSE_INTERVAL_SEC="${AUTOPROPOSE_INTERVAL_SEC:-20}"
POW_TARGET_PREFIX_BYTES="${POW_TARGET_PREFIX_BYTES:-}"
VALIDATOR_PUBKEYS_HEX="${VALIDATOR_PUBKEYS_HEX:-}"
...
install_liboqs_if_needed() {
  ...
  echo "liboqs_cmake_args=${LIBOQS_CMAKE_ARGS}"
  mkdir -p "${LIBOQS_BUILD_DIR}"
  cmake -S "${LIBOQS_SRC_DIR}" -B "${LIBOQS_BUILD_DIR}" -GNinja -DCMAKE_INSTALL_PREFIX="${LIBOQS_INSTALL_DIR}" ${LIBOQS_CMAKE_ARGS}
...
CFG
  ...
  if [ -n "${POW_TARGET_PREFIX_BYTES}" ]; then
    echo "pow_target_prefix_bytes=${POW_TARGET_PREFIX_BYTES}" >> "${CONF_FILE}"
  fi
  if [ -n "${VALIDATOR_PUBKEYS_HEX}" ]; then
    echo "validator_pubkeys_hex=${VALIDATOR_PUBKEYS_HEX}" >> "${CONF_FILE}"
  fi
}
